...
int main() {
    ...
    // Declare a queue
    amqp_queue_declare(conn, 1, amqp_cstring_bytes("consumer_queue"), 0, 1, 0, 1, amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    amqp_queue_bind(conn, 1, amqp_cstring_bytes("consumer_queue"), amqp_cstring_bytes("logs"),
    ...
    amqp_basic_consume(conn, 1, amqp_cstring_bytes("consumer_queue"), amqp_empty_bytes, 0, 1, 0, amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
