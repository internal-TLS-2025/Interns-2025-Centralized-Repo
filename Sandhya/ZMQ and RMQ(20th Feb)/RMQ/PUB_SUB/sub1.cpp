...
int main() {
    ...
    amqp_queue_declare(conn, 1, amqp_cstring_bytes("consumer1_queue"), 0, 1, 0, 1, amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    amqp_queue_bind(conn, 1, amqp_cstring_bytes("consumer1_queue"), amqp_cstring_bytes("logs"),
    ...
    amqp_basic_consume(conn, 1, amqp_cstring_bytes("consumer1_queue"), amqp_empty_bytes, 0, 1, 0, amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    std::cout << "Consumer 1: Waiting for messages..." << std::endl;
    ...
    while (true) {
        ...
        if (get_reply.reply_type == AMQP_RESPONSE_NORMAL) {
            std::cout << "Consumer 1: Received message: " 
