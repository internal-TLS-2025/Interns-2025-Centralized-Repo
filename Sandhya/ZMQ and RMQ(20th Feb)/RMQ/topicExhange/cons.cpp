...
int main() {
    ...
    // Bind the queue to the exchange with a routing key pattern (e.g., "logs.#")
    amqp_queue_bind(conn, 1, amqp_cstring_bytes("info_queue"), amqp_cstring_bytes("topic_logs"),
                    amqp_cstring_bytes("logs.#"), amqp_empty_table); // "logs.#" pattern
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    amqp_basic_consume(conn, 1, amqp_cstring_bytes("info_queue"), amqp_cstring_bytes("info"), 0, 1, 0, amqp_empty_table);
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
