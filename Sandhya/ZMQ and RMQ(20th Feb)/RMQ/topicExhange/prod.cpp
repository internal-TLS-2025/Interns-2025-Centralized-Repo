...
int main() {
    ...
    amqp_exchange_declare(conn, 1, amqp_cstring_bytes("topic_logs"), amqp_cstring_bytes("topic"),
                          0, 1, 0, 0, arguments); // Topic exchange
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    // Bind the queue to the exchange using a routing key (e.g., "logs.info")
    amqp_queue_bind(conn, 1, amqp_cstring_bytes("info_queue"), amqp_cstring_bytes("topic_logs"),
                    amqp_cstring_bytes("logs.info"), amqp_empty_table); // "logs.info" routing key
    reply = amqp_get_rpc_reply(conn);
    if (reply.reply_type != AMQP_RESPONSE_NORMAL) {
    ...
    // Publish a message with a topic-based routing key (e.g., "logs.info")
    amqp_basic_publish(conn, 1, amqp_cstring_bytes("topic_logs"), amqp_cstring_bytes("logs.info"),
